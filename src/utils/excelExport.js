...
/**
 * Export multiple statements into a single Excel workbook (one set of sheets per statement)
 * @param {Array<{transactions: Array, label?: string}>} statements
 * @param {string} filename
 */
export function exportMultipleStatements(statements, filename = 'bank_statements.xlsx') {
...
function appendStatementSheets({
  ...
  const transactionSheet = createTransactionSheet(transactions);
...
function createTransactionSheet(transactions) {
  const transactionData = formatTransactions(transactions);
  const ws = XLSX.utils.json_to_sheet(transactionData);

  ws['!cols'] = [
    { wch: 12 },  // Date
    { wch: 60 },  // Details (wider for descriptions)
    { wch: 15 },  // Debit
    { wch: 15 },  // Credit
    { wch: 15 },  // Balance
  ];
...
function formatTransactions(transactions) {
  return transactions.map(txn => ({
    Date: txn.date || '',
    Details: txn.details || '',
    'Debit (€)': txn.debit && parseFloat(txn.debit) > 0 ? parseFloat(txn.debit).toFixed(2) : '',
    'Credit (€)': txn.credit && parseFloat(txn.credit) > 0 ? parseFloat(txn.credit).toFixed(2) : '',
    'Balance (€)': txn.balance && parseFloat(txn.balance) !== 0 ? parseFloat(txn.balance).toFixed(2) : '',
  }));
}
...
function formatSummary(transactions) {
  ...
  return [
    ...
    { Metric: 'Total Income (€)', Value: `€${totalCredit.toFixed(2)}` },
    { Metric: 'Total Expenses (€)', Value: `€${totalDebit.toFixed(2)}` },
    { Metric: 'Net Amount (€)', Value: `€${netAmount.toFixed(2)}` },
    { Metric: 'Average Transaction (€)', Value: `€${((totalCredit + totalDebit) / transactions.length).toFixed(2)}` },
  ];
}
